...
// 키는 서버에만 두고, 브라우저에는 절대 노출되지 않습니다.
// Vercel > Settings > Environment Variables 에 GEMINI_API_KEY 등록 필요.
...
반드시 아래 JSON만 출력하십시오. 설명이나 코드펜스 없이 JSON 자체만.
{"topics":[{"title":"","summary":"","quote_ids":[],"opinions":[{"author_name":"","content":""}],"apply_note":null}]}`;

async function gemini(system, contents, json = false) {
  ...
  if (!r.ok) throw new Error(d?.error?.message || "Gemini 호출 실패");
  ...
  try {
    ...
    if (mode === "chat") {
      ...
      const shelf = quotes.length
        ? quotes.map(q => `- ${q.page ? `[${q.page}쪽] ` : ""}${q.content}${q.memo ? `\n  (독자 메모: ${q.memo})` : ""}`).join("\n")
        : "(아직 담아둔 문장이 없습니다.)";
      ...
      const contents = [
        ...
        { role: "model", parts: [{ text: "문장들 읽었습니다. 편하게 시작하시죠." }] },
        ...history.map(m => ({
      ...
      const text = await gemini(CHAT_SYSTEM, contents);
      return res.status(200).json({ text });
    }

    if (mode === "organize") {
      const { book, quotes = [], messages = [] } = payload;
      const qlist = quotes.length
        ? quotes.map(q => `${q.id}: ${q.page ? `[${q.page}쪽] ` : ""}${q.content}`).join("\n")
        : "(없음)";
      const talk = messages.map(m => `${m.who}: ${m.content}`).join("\n\n");

      const prompt =
        `책: 『${book.title}』${book.author ? ` — ${book.author}` : ""}\n\n` +
        `[담아둔 문장]\n${qlist}\n\n[대화 전문]\n${talk}`;

      const raw = await gemini(ORGANIZE_SYSTEM, [{ role: "user", parts: [{ text: prompt }] }], true);
      ...
      if (!topics.length) return res.status(502).json({ error: "정리할 주제를 찾지 못했습니다. 대화를 조금 더 나눠 보세요." });
